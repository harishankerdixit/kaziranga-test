...
@section('meta_description', 'Kaziranga Add Safari Block Date')
@section('content')
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="card my-4 shadow bg-body-tertiary br-1">
            <h3 class="card-header">Add Safari Block Date</h3>
            <div class="card-body py-0">
                <form method="post" action="{{ route('blockdates.add') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-2">
                        ...
                        <select class="form-control" id="zone" name="zone">
                            ...
                            <option value="Kaziranga Range, Kohora"
                                {{ old('zone') == 'Kaziranga Range, Kohora' ? 'selected' : '' }}>
                                Kaziranga Range, Kohora
                            </option>
                            <option value="Western Range, Bagori" {{ old('zone') == 'Western Range, Bagori' ? 'selected' : '' }}>
                                Western Range, Bagori
                            </option>
                            <option value="Burapahar Range, Ghorakati" {{ old('zone') == 'Burapahar Range, Ghorakati' ? 'selected' : '' }}>
                                Burapahar Range, Ghorakati
                            </option>
                            <option value="Eastern Range, Agaratoli" {{ old('zone') == 'Eastern Range, Agaratoli' ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 my-0 ps-0">
                            <div class="mb-2">
                                ...
                                <input type="date" class="form-control" id="from" name="from"
                                    value="{{ old('from') }}" placeholder="From Date....">
                                @error('from')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6 my-0 pe-0">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="date" class="form-control" id="to" name="to"
                                    value="{{ old('to') }}" placeholder="To Date....">
                                @error('to')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="mb-2">
                        ...
                        <textarea type="text" class="form-control" id="message" name="message" value="" placeholder="Message....">{{ old('message') }}</textarea>
                        @error('message')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
